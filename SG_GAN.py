...
from ops import conv2d, lrelu, instance_norm, Residual, upscale, avgpool2d, de_conv
...
class SG_GAN(object):
    ...
    def __init__(self, batch_size, max_iters, model_path, data_ob, sample_path, log_dir, learning_rate, is_load_, use_sp,
                 lam_recon, range_attri, beta1, beta2):
        ...
        self.y = tf.placeholder(tf.int32, [batch_size, self.range_attri])
        ...
        self.label_mask_3 = tf.placeholder(tf.float32, [batch_size])
        self.label_mask_4 = tf.placeholder(tf.float32, [batch_size])
        self.remove_unused_mask = tf.placeholder(tf.int32, [batch_size])
    ...
    def build_model_GAN(self):
        ...
        self.d_class_loss_2 = self.real_class_loss(self.D_real_class_logits_2, self.y_2, self.label_mask_2)
        self.d_class_loss_3 = self.real_class_loss(self.D_real_class_logits_3, self.y_3, self.label_mask_3)
        self.d_class_loss_4 = self.real_class_loss(self.D_real_class_logits_4, self.y_4, self.label_mask_4)
        ...
        self.m_vars = [var for var in self.t_vars if 'encode_decode' in var.name]

        print "d_vars", len(self.d_vars)
        print "e_vars", len(self.m_vars)
    ...
    def test(self):
        ...
        with tf.Session(config=config) as sess:
            ...
            self.saver.restore(sess, self.pixel_model_path + str(26000))
            ...
            for j in range(batch_num):
                ...
                x1_1, x1_2, x2_1, x2_2, x3_1, x3_2, x4_1, x4_2 = sess.run(
                ...
                for i in range(self.batch_size):
                    ...
                    save_images(batch_images_array[i:i + 1], [1, 1],
                                '{}/{:02d}_{:02d}real.png'.format(self.sample_path, j, i))
                    # r1_img = cv2.imread('{}/{:02d}real.png'.format(self.sample_path, 0), 1)
                    # cv2.imshow("real", r1_img)

                    # save_images(r1_1[0:self.batch_size], [self.batch_size/8, 8], '{}/{:02d}residual_img1_1.png'.format(self.sample_path, 0))
                    # r1_img = cv2.imread('{}/{:02d}residual_img1_1.png'.format(self.sample_path, 0), 1)
                    # cv2.imshow("residual_img1_1", r1_img)
                    #
                    # save_images(r1_2[0:self.batch_size], [self.batch_size/8, 8], '{}/{:02d}residual_img1_2.png'.format(self.sample_path, 0))
                    # r2_img = cv2.imread('{}/{:02d}residual_img1_2.png'.format(self.sample_path, 0), 1)
                    # cv2.imshow("residual_img1_2", r2_img)
                    #
                    # save_images(r2_1[0:self.batch_size], [self.batch_size/8, 8], '{}/{:02d}residual_img2_1.png'.format(self.sample_path, 0))
                    # r1_img = cv2.imread('{}/{:02d}residual_img2_1.png'.format(self.sample_path, 0), 1)
                    # cv2.imshow("residual_img2_1", r1_img)
                    #
                    # save_images(r2_2[0:self.batch_size], [self.batch_size/8, 8], '{}/{:02d}residual_img2_2.png'.format(self.sample_path, 0))
                    # r2_img = cv2.imread('{}/{:02d}residual_img2_2.png'.format(self.sample_path, 0), 1)
                    # cv2.imshow("residual_img2_2", r2_img)

                    save_images(x1_1[i:i + 1], [1, 1], '{}/{:02d}_{:02d}r1_1.png'.format(self.sample_path, j, i))
                    save_images(x1_2[i:i + 1], [1, 1], '{}/{:02d}_{:02d}r1_2.png'.format(self.sample_path, j, i))
                    save_images(x2_1[i:i + 1], [1, 1], '{}/{:02d}_{:02d}r2_1.png'.format(self.sample_path, j, i))
                    save_images(x2_2[i:i + 1], [1, 1], '{}/{:02d}_{:02d}r2_2.png'.format(self.sample_path, j, i))
                    save_images(x3_1[i:i + 1], [1, 1], '{}/{:02d}_{:02d}r3_1.png'.format(self.sample_path, j, i))
                    save_images(x3_2[i:i + 1], [1, 1], '{}/{:02d}_{:02d}r3_2.png'.format(self.sample_path, j, i))
                    save_images(x4_1[i:i + 1], [1, 1], '{}/{:02d}_{:02d}r4_1.png'.format(self.sample_path, j, i))
                    save_images(x4_2[i:i + 1], [1, 1], '{}/{:02d}_{:02d}r4_2.png'.format(self.sample_path, j, i))
    ...
    def train(self):
        ...
        opti_M = tf.train.AdamOptimizer(self.learning_rate * self.lr_decay, beta1=self.beta1, beta2=self.beta2).minimize(self.G_loss, var_list=self.m_vars)
        ...
        with tf.Session(config=config) as sess:
            ...
            while step <= self.max_iters:
                ...
                for i in range(5):
                    ...
                    train_data_list, train_lab_list1, train_lab_list2, train_lab_list3, train_lab_list4, batch_lab_mask_1,\
                        batch_lab_mask_2, batch_lab_mask_3, batch_lab_mask_4, batch_hair_mask \
                        = self.data_ob.getNextBatch(step_2, self.batch_size)

                    batch_images_array = self.data_ob.getShapeForData(train_data_list)
                    f_d = {self.images: batch_images_array, self.y_1: train_lab_list1, self.y_2: train_lab_list2,
                          self.y_3: train_lab_list3, self.y_4: train_lab_list4,
                         self.label_mask_1: batch_lab_mask_1, self.label_mask_2: batch_lab_mask_2,
                           self.label_mask_3: batch_lab_mask_3, self.label_mask_4: batch_lab_mask_4,
                           self.remove_unused_mask: batch_hair_mask, self.lr_decay: lr_decay}
                ...
                if step % 200 == 0:
                    ...
                    print("step %d D_loss =%.3f, D_loss_no_gp=%.3f \n"
                          "D_gan_loss_1 =%.3f, D_gan_loss_2=%.3f, D_gan_loss_3=%.3f, D_gan_loss_4=%.3f\n"
                          "G_loss=%.3f, G_class_loss=%.3f, G_gan_loss=%.3f"
                          "G_gan_loss_1=%.3f, G_gan_loss_2=%.3f, G_gan_loss_3=%.3f, G_gan_loss_4=%.3f, Recon_loss= %.3f\n"
                          "label_mask_1=%i" % (step, d_loss, d_loss_no_gp, d_gan_loss_1, d_gan_loss_2, d_gan_loss_3, d_gan_loss_4, g_loss, g_class_loss, g_gan_loss,
                                    g_gan_loss_1, g_gan_loss_2, g_gan_loss_3, g_gan_loss_4, recon_loss, np.sum(batch_lab_mask_1, dtype=np.int32)))
                ...
                if np.mod(step, 400) == 0:
                    ...
                    batch_images_array = np.clip(batch_images_array, -1, 1)
                    save_images(batch_images_array[0:self.batch_size], [self.batch_size/8, 8],
                    ...
                    # gender
                    save_images(x_tilde_1_1[0:self.batch_size], [self.batch_size/8, 8], '{}/{:02d}_x_tilde1_1.png'.format(self.sample_path, step))

                    save_images(x_tilde_1_2[0:self.batch_size], [self.batch_size/8, 8], '{}/{:02d}_x_tilde1_2.png'.format(self.sample_path, step))
                    # smile
                    save_images(x_tilde_2_1[0:self.batch_size], [self.batch_size/8, 8], '{}/{:02d}_x_tilde2_1.png'.format(self.sample_path, step))

                    save_images(x_tilde_2_2[0:self.batch_size], [self.batch_size/8, 8], '{}/{:02d}_x_tilde2_2.png'.format(self.sample_path, step))
                    # hair
                    save_images(x_tilde_3_1[0:self.batch_size], [self.batch_size/8, 8], '{}/{:02d}_x_tilde3_1.png'.format(self.sample_path, step))

                    save_images(x_tilde_3_2[0:self.batch_size], [self.batch_size/8, 8], '{}/{:02d}_x_tilde3_2.png'.format(self.sample_path, step))

                    #lipsticks
                    save_images(x_tilde_4_1[0:self.batch_size], [self.batch_size/8, 8], '{}/{:02d}_x_tilde4_1.png'.format(self.sample_path, step))

                    #save_images(b2_2[0:64], [8, 8], '{}/{:02d}_b2_2.png'.format(self.sample_path, step))
                    save_images(x_tilde_4_2[0:self.batch_size], [self.batch_size/8, 8], '{}/{:02d}_x_tilde4_2.png'.format(self.sample_path, step))

                if np.mod(step, 2000) == 0 and step != 0:
                    self.saver.save(sess, self.pixel_model_path + np.str(step))
                ...
                if np.mod(step, 1000) == 0:
                    ...
                    iters = len(self.data_ob.test_data_list) / self.batch_size
                    for i in range(iters):

                        test_data_list, test_lab_list1, test_lab_list2, _, test_lab_list4, _ = self.data_ob.getTestNextBatch(i, self.batch_size, is_shuffle=False)
                        ...
                        class_logits_on_data_1, class_logits_on_data_2, class_logits_on_data_4 = \
                            sess.run([self.D_real_class_logits_1, self.D_real_class_logits_2, self.D_real_class_logits_4], feed_dict={self.images: batch_images_array})
                    ...
                    print "Test Accuracy1", correct_1 / (np.float(iters * self.batch_size))
                    print "Test Accuracy2", correct_2 / (np.float(iters * self.batch_size))
                    print "Test Accuracy4", correct_4 / (np.float(iters * self.batch_size))

                    iters2 = len(self.data_ob.test_data_list_forhair) / self.batch_size
                    for i in range(iters2):
                        ...
                        batch_images_array = self.data_ob.getShapeForData(test_data_list)
                        class_logits_on_data_3 = sess.run(self.D_real_class_logits_3, feed_dict={self.images: batch_images_array})
                    ...
                    print "Test Accuracy3", correct_3/(np.float(iters2 * self.batch_size))
